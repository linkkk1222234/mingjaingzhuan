...
def generate_fusion_ideas(news_items: list[dict]) -> str:
    products = "\n".join([
        f"{i.get('no','')}. 【{i.get('title','')}】玩法:{i.get('gameplay','')}"
        for i in news_items if i.get("no", 0) != 99
    ])

    prompt = f"""
你是三国杀名将传策划总监。从今日资讯产品中选2款最具缝合潜力的，给出精炼方案。

【今日产品（只能从这里选）】
{products}

【背景】名将like = 卡牌+Roguelike，核心：组建6人名将阵容对战。
目标：找轻量钩子副玩法，D1冷启动→D2-D7过渡→D7+引入名将like核心。
关注：模拟/建造/经营/塔防/RPG/解谜元素，要求强手感、快反馈、易上手。
...
**钩子逻辑**：[用一句话说清楚：用什么钩子 → 怎么过渡到名将like]

🎣 **D1**：[玩家第一天的核心体验，15字以内]
🌉 **D2–D7**：[如何引入名将元素，15字以内]
⚔️ **D7+**：[名将like核心循环，15字以内]

**核心差异**：[与现有产品最大的不同，20字以内]

---

最后一行：**本期优选**：[产品名] — [理由，15字以内]
"""
    return ask_deepseek(
        prompt,
        system="你是手游策划总监，极度精炼，每个字都有价值，不说废话。",
        max_tokens=1500,
    )
...
def build_html(news_items: list[dict], fusion_text: str) -> str:
